...
if ($argc < 2) {
    echo <<<EOT
\033[35mUsage: ./level_stat_crontab.php <platform>
\033[0m
EOT;
    exit;
}
...
for ($userId = $minUserId; $userId <= $maxUserId; ++$userId) {
    ...
    $statArr['success'] += $creditModel->hGet(User::CREDIT_LEVEL_CHALLENGE, 0);
    $statArr['fail'] += $creditModel->hGet(User::CREDIT_LEVEL_FAIL, 0);
}
