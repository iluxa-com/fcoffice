...
class RedisHelper {
    ...
    public function getConn() {
        if (!isset($this->_r)) {
            ...
            if (!extension_loaded('redis')) {
                throw new UserException(UserException::ERROR_EXTENSION_NOT_LOADED, 'EXTENSION_NOT_LOADED', 'redis');
            }
            // 创建Redis的实例
            $this->_r = new Redis();
            try {
                ...
                throw new UserException(UserException::ERROR_REDIS, 'CANNOT_CONNECT_TO_REDIS', $e->getMessage());
            }
        }
        return $this->_r;
    }
    ...
    /**
     * Get the values related to the specified keys
     * @param array $keyArr
     * @return array
     */
     ...
     * @param string $key
     * @param int $offset
     * @param bool/int $value true/false/1/0
    ...
    public function getMultiple($keyArr) {
        return $this->getConn()->getMultiple($keyArr);
    }
    ...
    public function delete($key) {
        return call_user_func_array(array($this->getConn(), 'delete'), func_get_args());
    }
    ...
    public function substr($key, $start, $end) {
        return $this->getConn()->substr($key, $start, $end);
    }
    ...
    public function getKeys($pattern) {
        return $this->getConn()->getKeys($pattern);
    }
    ...
    public function lRemove($key, $value, $count) {
        return $this->getConn()->lRemove($key, $value, $count);
    }
    ...
    public function listTrim($key, $start, $stop) {
        return $this->getConn()->listTrim($key, $start, $stop);
    }
    ...
    public function lGet($key, $index) {
        return $this->getConn()->lGet($key, $index);
    }
    ...
    public function lGetRange($key, $start, $end) {
        return $this->getConn()->lGetRange($key, $start, $end);
    }
    ...
    public function sRemove($key, $member) {
        return $this->getConn()->sRemove($key, $member);
    }
    ...
    public function setTimeout($key, $ttl) {
        return $this->getConn()->setTimeout($key, $ttl);
    }
    ...
    public function flushDB() {
        throw new UserException(UserException::ERROR_REDIS, 'CALL_DISABLED_FUNCTION', __FUNCTION__);
    }
    ...
    public function select($dbindex) {
        ...
        // 不能选择DB,直接抛异常
        throw new UserException(UserException::ERROR_REDIS, 'CANNOT_SELECT_DB', $dbindex);
    }
    ...
    /**
     * Adds a value to the hash stored at key only if this field isn't already in the hash. If this value is already in the hash, FALSE is returned.
     * @param string $key
     * @param string $hashKey
     * @param string $value
    ...
    public function open($hos, $port = 6379, $timeout = 0) {
        return $this->getConn()->open($hos, $port, $timeout);
    }
    ...
    public function sGetMembers($key) {
        return $this->getConn()->sGetMembers($key);
    }
