...
class SNSTxWeibo {
    ...
    private function __getMultiInfo($fopenids) {
        ...
        $responseArr = $this->__doRequest('/t/user/multi_info');
        $extInfoArr = array();
    ...
    public function getUserInfo($snsUid = NULL) {
        ...
        $this->_paramArr = array();
        $responseArr = $this->__doRequest('/t/user/info');
        $returnArr = array(
    ...
    public function getFriends() {
        ...
        $responseArr = $this->__doRequest('/t/relation/friends');
        $allOpenidArr = array();
        foreach ($responseArr['items'] as $arr) {
    ...
    public function isMyFriend($friendSnsUid) {
        ...
        $responseArr = $this->__doRequest('/t/relation/is_friend');
    ...
    public function isContentValid($content) {
        ...
        $this->_paramArr = array(
            'content' => $content,
            'domain' => 3, // 1：Qzone； 2：腾讯朋友；3：腾讯微博；4：Q+平台；5：财付通开放平台；10：QQGame.
        );
