...
class SNSQzone {
    ...
    public function getUserInfo($snsUid = NULL) {
        ...
        $this->_paramArr = array();
        $responseArr = $this->__doRequest('/qzone/user/info');
        $returnArr = array(
    ...
    public function getFriends() {
        ...
        $responseArr = $this->__doRequest('/qzone/relation/friends');
        $allOpenidArr = array();
        foreach ($responseArr['items'] as $arr) {
    ...
    public function isMyFriend($friendSnsUid) {
        ...
        $responseArr = $this->__doRequest('/qzone/relation/is_friend');
    ...
    public function isContentValid($content) {
        ...
        $this->_paramArr = array(
            'content' => $content,
            'domain' => 1, // 1：Qzone； 2：腾讯朋友；3：腾讯微博；4：Q+平台；5：财付通开放平台；10：QQGame.
        );
